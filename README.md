...
- The functtion leans up the lemma information, adds the field name
  ZITAT, renames the field name UNTER_BEDEUTUNG to BEDEUTUNG and

- Entries that do not have a field name such as BEDEUTUNG or LEMMA are
  recursively appended to the last entry with a field name

- Replaces the field name ANHÄNGER with the field name of the last
  member of the U\*\_BEDEUTUNG-layers

- The function removes all elements in the quote column that do not
  belong to the actual quoted text.

- The function is not limited to a specific number of layers of meaning
  but dynamically adds as many layers of meaning as necessary.
...
- Removes all \\#/\\^\*:’; in the quote column. - All non-letters are
  removed from the reference, leaving only the first 3 words.

- The citation column is reduced to words longer than 3 letters and
...
- Splits lines containing multiple quotations into individual
  quotations.

- Adds missing references that were not recorded when entering multiple
  quotations in one column.
...
- Any Arabic numerals, annotations in parentheses () or curly braces {}
  in the lemma entry have no special meaning and can be deleted without
  further ado.

- All citations in the mlw files begin with an asterisk (\*).

- Entries with the following field names can be removed without further
  ado:
...
- Everything in columns containing quotations, between the quotation
  marks, belongs to the source reference.
